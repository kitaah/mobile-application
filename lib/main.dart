...
class _MyHomePageState extends State<MyHomePage> {
  ...
  Future<void> fetchTokenAndResources() async {
    try {
      final Dio dio = Dio();
      final responseToken = await dio.get('http://10.0.2.2:51525/api/token');
  ...
  Future<void> fetchResources(String token) async {
    try {
      final Dio dio = Dio();
      final response = await dio.get('http://10.0.2.2:51525/api/resources',
        ...
        setState(() {
          resources = jsonData.map<Resource>((item) {
            final imageUrl = item['image'].replaceAll(
                'https://web-application.ddev.site:8443', 'http://10.0.2.2:51525');
            return Resource(
              name: item['name'],
              imageUrl: imageUrl,
